import { initializeApp } from "https://www.gstatic.com/firebasejs/11.6.1/firebase-app.js";
       ...
       // --- LÓGICA DE FIRESTORE (LISTENERS) ---
        ...
        // --- RENDERIZADO DEL HISTORIAL (ACTUALIZADO: FOLIO REAL + HORA + LIMPIEZA UI) ---
        function renderReqs() {
            const term = document.getElementById('search-input').value.toLowerCase();
            
            // Filtramos por todo (incluyendo el folio_portal nuevo)
            const filtered = reqsData.filter(r => 
                (r.nombre||'').toLowerCase().includes(term) || 
                (r.empleadoId||'').toString().includes(term) || 
                (r.robot_target_employee||'').toString().includes(term) || 
                (r.articulo||'').toLowerCase().includes(term) || 
                (r.numRequi||'').toLowerCase().includes(term) ||
                (r.folio_portal||'').toLowerCase().includes(term) // <--- Busqueda por folio real
            );
            ...
            for (const r of filtered) {
                ...
                if (!reqGroups.has(r.numRequi)) { 
                    reqGroups.set(r.numRequi, { 
                        numRequi: r.numRequi, // Este es el ID interno para agrupar
                        fecha: r.fecha, 
            ...
            tbody.innerHTML = Array.from(reqGroups.values()).sort((a,b) => {
                // Intentamos ordenar por timestamp si existe para más precisión
                const timeA = a.items[0].timestamp ? (a.items[0].timestamp.seconds || 0) : 0;
                const timeB = b.items[0].timestamp ? (b.items[0].timestamp.seconds || 0) : 0;
                if (timeA && timeB) return timeB - timeA;
                return b.fecha.localeCompare(a.fecha);
            }).map(group => {
                let statusHtml = '';
                
                // 1. LÓGICA DE FOLIO A MOSTRAR
                // Si el primer item tiene folio_portal, mostramos ese. Si no, el REQ-XXX
                const folioDisplay = group.items[0].folio_portal || group.numRequi;
                const isPortalFolio = !!group.items[0].folio_portal; // Para darle un estilo diferente si quieres

                // 2. LÓGICA DE FECHA Y HORA
                let fechaHoraDisplay = group.fecha;
                const ts = group.items[0].timestamp;
                if (ts) {
                    // Convertimos el Timestamp de Firebase a Date JS
                    const dateObj = ts.toDate ? ts.toDate() : (new Date(ts.seconds * 1000));
                    // Formateamos: 15/01/2026 10:30
                    fechaHoraDisplay = dateObj.toLocaleString('es-MX', {
                        day: '2-digit', month: '2-digit', year: 'numeric',
                        hour: '2-digit', minute: '2-digit'
                    });
                }

                // 3. LÓGICA DE EMPLEADO (Sin etiqueta "Destino")
                ...
                if (group.targetId) {
                    ...
                    displayNombre = empObj ? empObj.nombre : "Empleado Nuevo/Externo";
                }
                ...
                // Color del avatar: Morado si es para otro (target), Gris si es normal.
                const avatarClass = isTarget ? 'bg-purple-100 text-purple-700 border-purple-200' : 'bg-gray-100 text-gray-600 border-gray-200';
                
                // Color del Folio: Verde si es de Portal, Azul si es local
                const folioClass = isPortalFolio ? 'text-green-700 bg-green-50 border-green-200' : 'text-primary-700 bg-primary-50 border-primary-100';

                return `
                <tr class="hover:bg-gray-50 transition border-b border-gray-100 last:border-0 group">
                    <td data-label="# Requi" class="px-6 py-4">
                        <span class="font-mono text-sm font-bold px-2 py-1 rounded border ${folioClass}">
                            ${folioDisplay}
                        </span>
                    </td>
                    
                    <td data-label="Fecha" class="px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-wide">
                        ${fechaHoraDisplay}
                    </td>
                    
                    <td data-label="Empleado" class="px-6 py-4">
                        <div class="flex items-center">
                            <div class="h-9 w-9 rounded-full flex-shrink-0 flex items-center justify-center text-sm font-bold mr-3 border ${avatarClass}">
                                ${displayNombre.charAt(0)}
                            </div>
                            <div>
                                <div class="font-bold text-gray-800 text-sm leading-tight">${displayNombre}</div>
                                <div class="text-xs text-gray-400 font-mono">${displayId}</div>
                                ${isTarget ? `<div class="text-[10px] text-gray-400 mt-0.5">Por: ${group.creadorNombre}</div>` : ''}
                            </div>
                        </div>
                    </td>
